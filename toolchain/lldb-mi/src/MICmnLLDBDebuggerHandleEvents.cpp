...
// 2021.3.15 Add support for OHOS target.
...
bool CMICmnLLDBDebuggerHandleEvents::HandleProcessEventStopReasonWatchpoint() {
  ...
  if (!CMIDriver::Instance().SetDriverStateRunningNotDebugging()) {
    ...
    SetErrorDescription(CMIUtilString::Format(
        MIRSRC(IDS_LLDBOUTOFBAND_ERR_SETNEWDRIVERSTATE),
        "HandleProcessEventStopReasonWatchpoint()", rErrMsg.c_str()));
    return MIstatus::failure;
  }
  ...
  return MiStoppedAtWatchPoint(brkPtId, brkPt);
}
...
bool CMICmnLLDBDebuggerHandleEvents::MiStoppedAtWatchPoint(
  ...
  if (nFrame == 0) {
    // MI print
    // *stopped,reason="breakpoint-hit",disp="del",bkptno="1",frame={level="0",addr="0x00000000ed2b8f00",func="void art::interpreter::ExecuteSwitchImplCpp<false, false>(art::interpreter::SwitchImplContext*)",args=[],file="??",fullname="??",line="-1"},thread-id="1",stopped-threads="all"
    // "*stopped,reason=\"breakpoint-hit\",disp=\"del\",bkptno=\"%d\",frame={},thread-id=\"%d\",stopped-threads=\"all\""
    const CMICmnMIValueConst miValueConst("watchpoint-hit");
    const CMICmnMIValueResult miValueResult("reason", miValueConst);
    CMICmnMIOutOfBandRecord miOutOfBandRecord(
        CMICmnMIOutOfBandRecord::eOutOfBand_Stopped, miValueResult);
    const CMICmnMIValueConst miValueConst2("del");

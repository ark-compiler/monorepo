...
foo({
AssertType(foo({    0: () => { },    ["hi" + "bye"]: true,    [0 + 1]: 0,    [+"hi"]: [0]}), "union");
AssertType(foo, "<T>(I<T>) => T");
AssertType({    0: () => { },    ["hi" + "bye"]: true,    [0 + 1]: 0,    [+"hi"]: [0]}, "{ [string]: union; [number]: union; 0: () => void; }");
...
g({ p: "" });
AssertType(g({ p: "" }), "string");
AssertType(g, "<T>(J<T>) => T");
AssertType({ p: "" }, "{ p: string; }");
AssertType(p, "string");
AssertType("", "string");

...
foo("", function* () {
AssertType(foo("", function* () {    yield* {        *[Symbol.iterator]() {            yield x => x.length        }    }}, p => undefined), "string");
AssertType(foo, "<T, U>(T, () => Iterable<(T) => U>, (U) => T) => T");
AssertType("", "string");
AssertType(function* () {    yield* {        *[Symbol.iterator]() {            yield x => x.length        }    }}, "() => Generator<(string) => number, void, undefined>");

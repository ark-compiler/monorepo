...
let a: { foo<T extends Array<string>>(x: T): string 
AssertType(a, "{ foo<T extends string[]>(T): string; }");

AssertType(foo, "<T extends string[]>(T) => string");

AssertType(x, "T");
}

let b = { foo<T extends RegExp>(x: T) { 
AssertType(b, "{ foo<T extends RegExp>(T): string; }");
AssertType({ foo<T extends RegExp>(x: T) { return ''; } }, "{ foo<T extends RegExp>(T): string; }");
AssertType(foo, "<T extends RegExp>(T) => string");
AssertType(x, "T");
AssertType('', "string");
return ''; } };

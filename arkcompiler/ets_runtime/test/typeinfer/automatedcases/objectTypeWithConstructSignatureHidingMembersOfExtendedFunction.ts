...
let r1d = i.data;
AssertType(r1d, "number");
AssertType(i.data, "number");

let r1e = i['hm']; // should be Object
AssertType(r1e, "error");
AssertType(i['hm'], "error");
AssertType(i, "I");
AssertType('hm', "string");

let x: {
AssertType(x, "{ new (): number; apply(any, ?any): void; call(number, ...number[]): any; }");
...
let r2d = x.data;
AssertType(r2d, "number");
AssertType(x.data, "number");

let r2e = x['hm']; // should be Object
AssertType(r2e, "error");
AssertType(x['hm'], "error");
AssertType(x, "{ new (): number; apply(any, ?any): void; call(number, ...number[]): any; }");
AssertType('hm', "string");

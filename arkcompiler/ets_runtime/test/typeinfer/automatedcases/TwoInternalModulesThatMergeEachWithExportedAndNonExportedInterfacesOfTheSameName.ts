...
let p: { x: number; y: number; toCarth(): A.Point; };
AssertType(p, "{ x: number; y: number; toCarth(): A.Point; }");
AssertType(x, "number");
AssertType(y, "number");
...
// ensure merges as expected
let l: { new (s: A.Point, e: A.Point); 
AssertType(l, "new (A.Point, A.Point) => any");

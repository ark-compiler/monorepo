...
function fn<T>() {
    let values: ISubject<any>[] = [];
AssertType(values, "ISubject<any>[]");
AssertType([], "undefined[]");

    // Hang when using <T>, but not <any>
    combineLatest<T>(values);
AssertType(combineLatest<T>(values), "void");
AssertType(combineLatest, "{ <TOther>(IObservable<TOther>[]): void; (): void; }");
AssertType(values, "ISubject<any>[]");
}

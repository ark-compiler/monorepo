...
void AssemblerModule::Run(const CompilationConfig *cfg, Chunk* chunk)
{
    ...
    else if(cfg->IsRiscv64())
    {
        GenerateStubsRiscv64(chunk);
    }
    else 
    {
        LOG_ECMA(FATAL) << "this branch is unreachable30";
        UNREACHABLE();
    }
}
...
void AssemblerModule::GenerateStubsRiscv64(Chunk* chunk)
{
    riscv64::ExtendedAssembler assembler(chunk, this);
    LOG_COMPILER(INFO) << "================ compiling asm stubs ================";
    for (size_t i = 0; i < asmCallSigns_.size(); i++) {
        auto cs = asmCallSigns_[i];
        ASSERT(cs->HasConstructor());
        ...
        AssemblerStub *stub = static_cast<AssemblerStub*>(
            cs->GetConstructor()(nullptr));
        stub->GenerateRiscv64(&assembler);
        delete stub;
    }
    buffer_ = assembler.GetBegin();
    bufferSize_ = assembler.GetCurrentPosition();
...
#define DECLARE_JSCALL_TRAMPOLINE_RISCV64_GENERATE(name)                                                \
void name##Stub::GenerateRiscv64(Assembler *assembler)                                                  \
{                                                                                                       \
    riscv64::ExtendedAssembler *assemblerRiscv64 = static_cast<riscv64::ExtendedAssembler*>(assembler); \
    riscv64::OptimizedCall::name(assemblerRiscv64);                                                     \
}

#define DECLARE_FAST_CALL_TRAMPOLINE_RISCV64_GENERATE(name)                                             \
void name##Stub::GenerateRiscv64(Assembler *assembler)                                                  \
{                                                                                                       \
    riscv64::ExtendedAssembler *assemblerRiscv64 = static_cast<riscv64::ExtendedAssembler*>(assembler); \
    riscv64::OptimizedFastCall::name(assemblerRiscv64);                                                 \
}

#define DECLARE_ASM_INTERPRETER_TRAMPOLINE_RISCV64_GENERATE(name)                                       \
void name##Stub::GenerateRiscv64(Assembler *assembler)                                                  \
{                                                                                                       \
    riscv64::ExtendedAssembler *assemblerRiscv64 = static_cast<riscv64::ExtendedAssembler*>(assembler); \
    riscv64::AsmInterpreterCall::name(assemblerRiscv64);                                                \
}
...
JS_CALL_TRAMPOLINE_LIST(DECLARE_JSCALL_TRAMPOLINE_RISCV64_GENERATE)
FAST_CALL_TRAMPOLINE_LIST(DECLARE_FAST_CALL_TRAMPOLINE_RISCV64_GENERATE)
ASM_INTERPRETER_TRAMPOLINE_LIST(DECLARE_ASM_INTERPRETER_TRAMPOLINE_RISCV64_GENERATE)
...
#undef DECLARE_JSCALL_TRAMPOLINE_RISCV64_GENERATE
#undef DECLARE_FAST_CALL_TRAMPOLINE_RISCV64_GENERATE
#undef DECLARE_ASM_INTERPRETER_TRAMPOLINE_RISCV64_GENERATE

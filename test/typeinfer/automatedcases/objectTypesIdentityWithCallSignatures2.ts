...
let a: { foo(x: Date): string 
AssertType(a, "{ foo(Date): string; }");

AssertType(foo, "(Date) => string");

AssertType(x, "Date");
}

let b = { foo(x: RegExp) { 
AssertType(b, "{ foo(RegExp): string; }");
AssertType({ foo(x: RegExp) { return ''; } }, "{ foo(RegExp): string; }");
AssertType(foo, "(RegExp) => string");
AssertType(x, "RegExp");
AssertType('', "string");
return ''; } };

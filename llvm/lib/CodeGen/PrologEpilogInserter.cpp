...
void PEI::RecordCalleeSaveRegisterAndOffset(MachineFunction &MF, const std::vector<CalleeSavedInfo> &CSI)
{
  ...
  if ((archType != Triple::aarch64 && archType != Triple::x86_64 && archType != Triple::riscv64) || !(TFI->hasFP(MF))) {
    return;
  }
  unsigned FpRegDwarfNum = 0;
  if (archType == Triple::aarch64) {
    FpRegDwarfNum = 29; // x29
  } else if (archType == Triple::riscv64) {
    FpRegDwarfNum = 8; // x8,s0
  } else {
    FpRegDwarfNum = 6; //rbp
  }
  int64_t FpOffset = 0;
  ...
  for (std::vector<CalleeSavedInfo>::const_iterator
    ...
    if ((DwarfRegNum == FpRegDwarfNum) && (archType == Triple::riscv64)) {
      continue;
    }
...
void PEI::calculateFrameObjectOffsets(MachineFunction &MF) {
  ...
  if (archType == Triple::riscv64 && TFI.hasFP(MF)) {
    int fpPosition = TFI.GetFixedFpPosition();
    int slotSize = sizeof(uint64_t);
    int fpToCallerSpDelta = 0;
    if (fpPosition >= 0) {
